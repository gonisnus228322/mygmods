if BLOOD_COLOR_THEBOYSBASE_SYNTH == nil then BLOOD_COLOR_THEBOYSBASE_SYNTH = 7 end

local function getGoreCVarFloat(name, fallback, minValue, maxValue)
    local cvar = GetConVar(name) or GetConVar(string.Replace(name, "theboysbase", "homelander"))
    local value = cvar and cvar:GetFloat() or fallback
    return math.Clamp(value, minValue, maxValue)
end

local function getGoreCVarBool(name, fallback)
    local cvar = GetConVar(name) or GetConVar(string.Replace(name, "theboysbase", "homelander"))
...
local blood_colors = {
    ...
    [BLOOD_COLOR_THEBOYSBASE_SYNTH] = Color(165,175,150),
}

local splatter_distance = 50
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function EFFECT:Init( data )
    if not getGoreCVarBool("theboysbase_cl_blood_enabled", true) then return end

    local blood_color = data:GetFlags()
    local blood_color_tbl = blood_colors[ blood_color ]
    if !blood_color_tbl then return end

    local amountScale = getGoreCVarFloat("theboysbase_cl_blood_amount", getGoreCVarFloat("theboysbase_cl_gore_amount", 1, 0, 2), 0, 2)
    ...
    for i = 1, math.floor(14 * amountScale + 0.5) do
